...
WorldView::WorldView(QWidget *parent, B9Layout* main) : QOpenGLWidget(parent)
{
	pMain = main;
...
void WorldView::initializeGL()
{
    ...
    glClearColor(0.0f, 0.39f, 0.39f, 1.0f);
...
void WorldView::ResetCamera(bool xrayon)
{
    ...
    if(perspective)
    {
        ...
        // TODO: Replace with RHI projection matrix
        // gluPerspective(30,double(width())/height(),nearClip,5500);
        // For now, set up basic perspective projection manually
        glMatrixMode(GL_PROJECTION);
        glLoadIdentity();
...
{
    camdistTarget -= event->angleDelta().y()/8.0;
    //变焦范围
    if(perspective)
    {
