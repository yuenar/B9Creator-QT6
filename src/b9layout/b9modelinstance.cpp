 ...
 * 模块名称:    b9layout
 * 文件名:     b9modelinstance.cpp
 * 模块功能:   模型实例类实现文件，包含3D模型实例的具体实现
 * 创建者:    owenzhang
 * 创建日期:    2026-03-15
 * 版本号:     V1.0.0
 * 历史记录:
...
void B9ModelInstance::FlipSupports()
{
    ...
    for(s = 0; s < supportStructureList.size(); s++)
    {
        ...
        //顶部
        oldTopLocalPosition = cSprt->GetTopPoint();
        distToPlane = QVector3D().distanceToLine(QVector3D(QVector2D(oldTopLocalPosition)), mirrorVec);
        side = QVector3D::crossProduct(mirrorVec, QVector3D(QVector2D(oldTopLocalPosition).normalized())).z();
        if(side < 0) side = -1.0;
        else side = 1.0;
        ...
        //底部
        oldBottomLocalPosition = cSprt->GetBottomPoint();
        distToPlane = QVector3D().distanceToLine(QVector3D(QVector2D(oldBottomLocalPosition)), mirrorVec);
        side = QVector3D::crossProduct(mirrorVec, QVector3D(QVector2D(oldBottomLocalPosition).normalized())).z();
        if(side < 0) side = -1.0;
        else side = 1.0;
        ...
        distToPlane = QVector3D().distanceToLine(QVector3D(QVector2D(TopNorm)), mirrorVec);
        side = QVector3D::crossProduct(mirrorVec, QVector3D(QVector2D(TopNorm).normalized())).z();
        if(side < 0) side = -1.0;
        else side = 1.0;
        ...
        distToPlane = QVector3D().distanceToLine(QVector3D(QVector2D(BottomNorm)), mirrorVec);
        side = QVector3D::crossProduct(mirrorVec, QVector3D(QVector2D(BottomNorm).normalized())).z();
        if(side < 0) side = -1.0;
        else side = 1.0;
