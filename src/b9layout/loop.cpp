...
bool Loop::formTriStrip()
{
    ...
    // TODO: Replace with modern RHI triangulation
    // As of 1.5.2 use the glu tesselator to form triangles
    // For now, return empty to avoid GLU dependency
    // B9Tesselator tess;
    // return tess.Triangulate(&polygonStrip,&triangleStrip);
    
    // Simple fallback: create basic triangles from polygon
    if(polygonStrip.size() < 3) return false;
    
    for(int i = 1; i < polygonStrip.size() - 1; i++) {
        triangleStrip.push_back(polygonStrip[0]);
        triangleStrip.push_back(polygonStrip[i]);
        triangleStrip.push_back(polygonStrip[i+1]);
    }
