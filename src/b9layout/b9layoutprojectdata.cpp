 ...
 * 模块名称:    b9layout
 * 文件名:     b9layoutprojectdata.cpp
 * 模块功能:   布局项目数据类实现文件，包含布局项目数据的具体实现
 * 创建者:    owenzhang
 * 创建日期:    2026-03-15
 * 版本号:     V1.0.0
 * 历史记录:
...
//////////////////////////////////
    ...
    //文件访问:
    void B9LayoutProjectData::New()//清除了已存在内部工程的数据，创建一个新使用默认值
	{
        LoadDefaults();
        SetDirtied(false);
	}
    bool B9LayoutProjectData::Open(QString filepath, bool withoutVisuals) //返回成功
    {
        int i;
        int FileVersion;
		bool jumpout = false;
		...
		while(!jumpout)
		{
			...
			if(buff != "endinstancelist")
			{
                ...
                B9ModelInstance* pinst = pMain->AddModel(modelpath,withoutVisuals);//如果它无法找到该文件，pinst为空。
    ...
    bool B9LayoutProjectData::Save(QString filepath) //返回成功
	{
        unsigned int i,j;
    ...
    QString B9LayoutProjectData::GetFileName()//未保存过则无标题
	{
        return mfilename;
	}
