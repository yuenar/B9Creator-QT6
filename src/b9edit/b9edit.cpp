...
void B9Edit::ShowSliceWindow()
{
    ...
    if(!pEditView->isMaximized() && pEditView->pDrawingContext->pActiveImage)
        pEditView->resize(pEditView->pDrawingContext->pActiveImage->width() + 200, pEditView->pDrawingContext->pActiveImage->height() + 100);


    if(QGuiApplication::primaryScreen()->availableGeometry().width() < pEditView->size().width()+100 || QGuiApplication::primaryScreen()->availableGeometry().height() < pEditView->size().height()+100)
    {
