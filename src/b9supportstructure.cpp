 ...
 * 模块名称:    model
 * 文件名:     b9supportstructure.cpp
 * 模块功能:   支撑结构类实现文件，包含3D打印支撑结构的具体实现
 * 创建者:    owenzhang
 * 创建日期:    2026-03-15
 * 版本号:     V1.0.0
 * 历史记录:
...
void B9SupportAttachmentData::CenterGeometry()
{
    ...
    for(t = 0; t < triList.size(); t++)
    {
        ...
        triList[t].UpdateBounds(); // 因为我们正移动着每一个三角数据，我们需要更新自己的界限了。
    }
}
...
void B9SupportAttachmentData::FormDisplayList()
{
    ...
    for(t = 0; t < triList.size(); t++)//遍历全部三角数据
    {
...
{
    QVector3D topFinalNormal;//结合角度系数与源始法线的结果。
    ...
    //计算PenetrationPoints（即真正的终点 渲染/承重）
