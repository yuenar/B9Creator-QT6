...
void B9Projector::createToverMap0()
{
    ...
    //简单边缘检测，设定像素到255，如果“挨着的”（上，下，左，右）是黑像素
    m_vToverMap.resize(width*height);
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
...
void B9Projector::drawCBM()
{
    ...
    if(m_iLevel<0)  // 层叠和规范切片，只要我们还没有开始清理它
    {
        // 层叠切片
...
{	
	...
	emit newGeometry (QGuiApplication::screens().indexOf(screen), geometry());
}
